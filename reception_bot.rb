...
class ReceptionBot
  ...
  def muc_ensure_room(room_name, &block)
    ...
    muc = muc_for_room(room_name)
    if muc      
      block.call(muc) if block
      return      
    end      
  ...
  def connect_user_with_operator(user)    
    ...
    if free_op
      ...
      muc_ensure_room(room_name) {|muc|
        muc_invite(user, room_name) unless muc.bare_xjids.include? JID.new(user).bare        
        muc_invite(free_op, room_name)  unless muc.bare_xjids.include? JID.new(free_op).bare
      }
    else      
  ...
  def process_leave(muc, pres)
    muc.roster.delete pres.from.resource
    prompt_after {    
      user_role = classify_jid(pres.xjid)    
      ...
      case user_role      
        ...
        if jids_left.values.include? :operator
          ...
          puts("customer left -> release room operators #{freed_ops}")
